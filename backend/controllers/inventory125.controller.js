const inventory125Service = require("../services/inventory125.service");

class Inventory125Controller {
  // Add a new medication
  async addMedication(req, res) {
    try {
      const medicationData = req.body;
      console.log("Received medication data:", medicationData);
      const result = await inventory125Service.addMedication(medicationData);
  ...
  async getMedications(req, res) {
    try {
      const medications = await inventory125Service.getMedications();
      res.status(200).json({
        status: "success",
        data: medications,
      });
  ...
  async getMedicationById(req, res) {
    try {
      ...
      const medication = await inventory125Service.getMedicationById(medicationId);
      if (!medication) {
  ...
  async updateMedication(req, res) {
    try {
      ...
      const result = await inventory125Service.updateMedication(medicationId, medicationData);
  ...
  async deleteMedication(req, res) {
    try {
      const medicationId = req.params.id;
      const result = await inventory125Service.deleteMedication(medicationId);
...
module.exports = new Inventory125Controller();