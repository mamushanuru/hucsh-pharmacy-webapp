const inventory92Service = require("../services/inventory92.service");

class Inventory92Controller {
  // Add a new medication
  async addMedication(req, res) {
    try {
      const medicationData = req.body;
      console.log("Received medication data:", medicationData);
      const result = await inventory92Service.addMedication(medicationData);
  ...
  async getMedications(req, res) {
    try {
      const medications = await inventory92Service.getMedications();
      res.status(200).json({
        status: "success",
        data: medications,
      });
  ...
  async getMedicationById(req, res) {
    try {
      ...
      const medication = await inventory92Service.getMedicationById(medicationId);
      if (!medication) {
  ...
  async updateMedication(req, res) {
    try {
      ...
      const result = await inventory92Service.updateMedication(medicationId, medicationData);
  ...
  async deleteMedication(req, res) {
    try {
      const medicationId = req.params.id;
      const result = await inventory92Service.deleteMedication(medicationId);
...
module.exports = new Inventory92Controller();