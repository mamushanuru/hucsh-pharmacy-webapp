...
const AdminDashboard92 = () => {
  ...
  const handleUpdateMedication = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5050/api/inventory92/medications/${editMedication.medication_id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteMedication = async (id) => {
    try {
      const response = await fetch(`http://localhost:5050/api/inventory92/medications/${id}`, {
        method: "DELETE",
      });
