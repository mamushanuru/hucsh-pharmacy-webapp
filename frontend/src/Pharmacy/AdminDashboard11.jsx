...
const AdminDashboard11 = () => {
  ...
  // Use the custom socket hook with pharma_id = 1
  const socket = useSocket(1);

  // Fetch medications from the backend
  useEffect(() => {
  // In AdminDashboard11.jsx - Update the fetchMedications function
const fetchMedications = async () => {
  try {
    const token = localStorage.getItem('employee_token'); // Get stored token
    const response = await fetch(
      `${import.meta.env.VITE_REACT_APP_API_URL}/api/inventory11/medications`,
      {
        headers: {
  ...
  useEffect(() => {
    ...
    const handleMedicationChange = (data) => {
      // Only process updates for this pharmacy (pharma_id = 1)
      if (data.pharma_id !== 1) return;
   ...
   const handleAddMedication = async (e) => {
     ...
     try {
       ...
       const response = await fetch(`${import.meta.env.VITE_REACT_APP_API_URL}/api/inventory11/medications`, {
        method: "POST",
        headers: { 
   ...
   const handleUpdateMedication = async (e) => {
     ...
     try {
       ...
       const response = await fetch(
        `${import.meta.env.VITE_REACT_APP_API_URL}/api/inventory11/medications/${editMedication.medication_id}`,
        {
          method: "PUT",
          headers: { 
            "Content-Type": "application/json",
   ...
   const handleDeleteMedication = async (id) => {
     try {
      const response = await fetch(
        `${import.meta.env.VITE_REACT_APP_API_URL}/api/inventory11/medications/${id}`,
        {
          method: "DELETE",
          headers: {
            "Authorization": `Bearer ${localStorage.getItem('employee_token')}`
