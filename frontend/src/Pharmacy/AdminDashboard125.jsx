...
const AdminDashboard125 = () => {
  ...
  useEffect(() => {
    const fetchMedications = async () => {
      try {
        const response = await fetch("http://localhost:5000/api/inventory125/medications");
  ...
  const handleUpdateMedication = async (e) => {
    ...
    try {
      ...
      const response = await fetch(
        `http://localhost:5000/api/inventory125/medications/${editMedication.medication_id}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteMedication = async (id) => {
    try {
      const response = await fetch(`http://localhost:5000/api/inventory125/medications/${id}`, {
        method: "DELETE",
      });
